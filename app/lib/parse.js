...
let provinces, cities, areas, provinceString, cityString, areaString
...
const AddressParse = (address, options) => {
    const { type = 1, extraGovData = {}, textFilter = [], nameMaxLength = 4 } = typeof options === 'object' ? options : (typeof options === 'number' ? { type: options } : {})

    if (!AddressParse.isInit) {
        return {}
    }
...
AddressParse.init = (data) => {
    ...
    AddressParse.isInit = true
    AddressParse.provinces = provinces
    AddressParse.cities = cities
    AddressParse.areas = areas
}
